...
def handle_client(conn):
    ...
    # If client validation is successful, enter the main loop
    try:
        while True:
            # Send menu message, encrypted with sym_key
            ...
            if response == '1':
                ...
                viewing_email_subprotocol(conn)  # There was a typo in your original method name
            elif response == '4':
...
def start_server():
    ...
    server_socket.listen(5)  # Listen for incoming connections, increased backlog to 5
        ...
        while True:
            ...
            if pid == 0:  # Child process
                ...
                if user_exists:
                    ack = decrypt_data_with_sym(conn.recv(1024))
                    if ack == "OK":
                        handle_client(conn)  # Implement client handling logic in a function for clarity
                        
                conn.close()
                os._exit(0)  # Child process exits here
            else:  # Parent process
                conn.close()  # Close the client socket in the parent process
